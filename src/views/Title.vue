<template>
  <div>
    ...
    <div style="margin: 10px 0">
      <el-input style="width: 300px" placeholder="请输入职称名查询" suffix-icon="el-icon-search" v-model="title"></el-input>
...
<!--          </el-popconfirm>-->
    ...
    <el-dialog title="职称信息" :visible.sync="dialogFormVisible" width="30%">
...
export default {
  ...
  methods:{
    ...
    delBatch(){
      ...
      //map(v=>v.id)  [{},{},{}]=>[1,2,3]
      // //console.log(ids);
      this.request.post("/title/del/batch",ids).then(res=>{
        if(res.data){
    ...
    del(id){
      this.request.delete("/title/"+id).then(res=>{
        if(res.data){
          this.$message.success("删除成功");
        }else {
          this.$message.error("删除失败");
